/**
 * Aspect-ratio QA v294b — staff home + kid home only.
 *   node scripts/qa-agent-aspects-v294b.mjs
 *
 * Output: .qa-screens/agent-aspects-v294b/{viewport}/
 */
...
const OUT = path.join(ROOT, '.qa-screens', 'agent-aspects-v294b');
...
async function inspect(page, viewport, screen) {
  const data = await page.evaluate((railSel) => {
    ...
    // Text clip: headings/cards with ellipsis or overflow hidden and scrollWidth > client
    const textClip = [];
    ...
    // Grid overlap: siblings in grid with negative gap feel — cells overlapping
    ...
    // Anti-slop heuristics (P2 visual notes)
    ...
    for (const el of document.querySelectorAll('#view *, header.app-chrome *')) {
      ...
      if (/rgb\(.*(?:1[0-9]{2}|2[0-4][0-9]).*?(?:1[0-9]{2}|2[0-4][0-9]).*?(?:2[0-4][0-9]|25[0-5])/.test(bg) === false) {
        if (/violet|purple|#7c3aed|#8b5cf6|147,\s*51,\s*234|139,\s*92,\s*246/i.test(bg)) slop.purple++;
      }
...
async function runScreen(ctx, page, vp, screenId, setupFn, filename) {
  await setupFn(page);
...
for (const vp of VIEWPORTS) {
  ...
  const staffResult = await runScreen(staffCtx, staffPage, vp, 'staff-home', staffHome, 'staff-home.png');
  ...
  await boot(kidPage);
  const kidResult = await runScreen(kidCtx, kidPage, vp, 'kid-home', kidHome, 'kid-home.png');
  await kidCtx.close();
...
let md = `# Aspect-ratio QA v294b — staff + kid home\n\n`;
md += `- **When:** ${reportJson.at}\n`;
md += `- **Base:** ${BASE}/m/\n`;
md += `- **Auth:** e4 staff, k1 child\n\n`;
md += `| Viewport | Size | Result | P0 | P1 | P2 | Staff | Kid |\n`;
md += `|----------|------|--------|----|----|----|-------|-----|\n`;
for (const r of summaryRows) {
  md += `| ${r.label} | ${r.w}×${r.h} | **${r.pass ? 'PASS' : 'FAIL'}** | ${r.p0} | ${r.p1} | ${r.p2} | \`${r.staffHome}\` | \`${r.kidHome}\` |\n`;
}
md += `\n## Issues\n\n`;
for (const r of summaryRows) {
